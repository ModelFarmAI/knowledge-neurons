...
def apply_chunking_to_forward(
    forward_fn: Callable[..., torch.Tensor], chunk_size: int, chunk_dim: int, *input_tensors, tgt_pos=None, tmp_score=None, imp_pos=None, imp_op=None
) -> torch.Tensor:
    """
    ...
    if num_args_in_forward_chunk_fn != len(input_tensors) + 4:
        raise ValueError(
            f"forward_chunk_fn expects {num_args_in_forward_chunk_fn} arguments, but only {len(input_tensors) + 4} input "
            "tensors are given"
        )
    ...
    return forward_fn(*input_tensors, tgt_pos=tgt_pos, tmp_score=tmp_score, imp_pos=imp_pos, imp_op=imp_op )
...
class BertIntermediate(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor, tgt_pos = None, tmp_score=None, imp_pos=None, imp_op=None,) -> torch.Tensor:
        hidden_states = self.dense(hidden_states)
...
class BertLayer(nn.Module):
    ...
    def forward(
        ...
        if imp_op == 'return':
            layer_output, intermediate_output, imp_weights = apply_chunking_to_forward(
            self.feed_forward_chunk, self.chunk_size_feed_forward, self.seq_len_dim, attention_output, tgt_pos=tgt_pos, tmp_score=tmp_score, imp_pos=imp_pos, imp_op=imp_op
        )
        else:
    ...
    def feed_forward_chunk(self, attention_output, tgt_pos=None, tmp_score=None, imp_pos=None, imp_op=None ):
        if imp_op == 'return':
